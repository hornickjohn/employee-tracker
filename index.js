...
//input validation to prevent an employee from managing itself
const RequireDifferentEmployee = (inp,ans) => ans.employee !== inp;
...
async function UpdateData(table,reftable) {
    ...
    if(table.name === 'employees' && reftable.name === 'employees') {
        ...
        questions.push({ name:'manager', message:'Who is the new manager?', type:'list', choices:await GetInquirerOptionsFromTable(reftable,['first_name','last_name']), validate:RequireDifferentEmployee });
    ...
    .then(async answers => {
        ...
        if(table.name === 'employees' && reftable.name === 'employees') {
            table.updateData(answers.employee,'manager_id',answers.manager);
        } else if(table.name === 'employees' && reftable.name === 'roles') {
            table.updateData(answers.employee,'role_id',answers.role);
        } else if(table.name === 'roles' && reftable.name === 'departments') {
            table.updateData(answers.role,'department_id',answers.department);
        }
    })
    .catch((err) => {
        if (err) {
